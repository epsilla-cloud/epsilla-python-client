...
class VectorRetriever:
    ...
    def retrieve(self, query: str) -> list[dict]:
        ...
        if status_code != 200:
            error_msg = (
                response["message"] if "message" in response else "Unknown error"
            )
            raise Exception(
                f"Failed to retrieve data from table {self._table_name}: {error_msg}"
            )
        ...
        for record in response["result"]:
            ...
            if self._primary_key_field not in record:
                raise Exception(
                    f"Primary key field {self._primary_key_field} not found in the response from table {self._table_name}"
                )
...
class RelativeScoreFusionReranker(Reranker):
    ...
    def normalize_distances(self, candidates: list[dict]) -> list[dict]:
        ...
        for candidate in candidates:
            normalized_score = (candidate["@distance"] - min_distance) / (
                max_distance - min_distance
            )
            normalized_candidates.append(
                {"candidate": candidate, "score": 1 - normalized_score}
            )
    ...
    def rerank(self, candidates: list[list[dict]]) -> list[dict]:
        normalized_lists = [
            self.normalize_distances(candidate_list) for candidate_list in candidates
        ]
        ...
        sorted_candidates = sorted(
            aggregated_scores.values(), key=lambda x: x["score"], reverse=True
        )
...
class DistributionBasedScoreFusionReranker(Reranker):
    ...
    def normalize_distances(
        self, scale: list[float], candidates: list[dict]
    ) -> list[dict]:
        # Normalize distances: (distance - min_distance) / (max_distance - min_distance)
        normalized_candidates = []
        for candidate in candidates:
            normalized_score = max(candidate["@distance"] - scale[0], 0) / (
                scale[1] - scale[0]
            )
            normalized_candidates.append(
                {"candidate": candidate, "score": 1 - min(1, normalized_score)}
            )
    ...
    def rerank(self, candidates: list[list[dict]]) -> list[dict]:
        normalized_lists = [
            self.normalize_distances(self._scale_ranges[i], candidate_list)
            for i, candidate_list in enumerate(candidates)
        ]
...
class SearchEngine:
    ...
    def set_reranker(
        ...
        if type == "rrf" or type == "reciprocal_rank_fusion":
            ...
            if len(scale_ranges) != len(self._retrievers):
                raise Exception(
                    "The length of scale_ranges should be equal to the number of retrievers"
                )
            self._reranker = DistributionBasedScoreFusionReranker(
                scale_ranges, limit=limit
            )
        else:
    ...
    def search(self, query: str) -> list[dict]:
        ...
        if len(self._retrievers) > 1 and not self._reranker:
            raise Exception(
                "More than one retriever added to the search engine, but no reranker is set"
            )
